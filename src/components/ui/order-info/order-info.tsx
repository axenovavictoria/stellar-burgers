...
export const OrderInfoUI: FC<OrderInfoUIProps> = memo(({ orderInfo }) => (
  <div className={styles.wrap}>
    <h3
      className={clsx('text text_type_main-medium pb-3 pt-10', styles.header)}
    >
      {orderInfo.name}
    </h3>
    ...
    <ul className={clsx(styles.list, 'mb-8')}>
      {Object.values(orderInfo.ingredientsInfo).map((item, index) => (
        <li className={clsx('pb-4 pr-6', styles.item)} key={index}>
          ...
          <span
            className={clsx(
              'text text_type_digits-default pl-4 pr-4',
              styles.quantity
            )}
          >
    ...
    <div className={styles.bottom}>
      ...
      <span
        className={clsx('text text_type_digits-default pr-4', styles.total)}
      >
        {orderInfo.total}
      </span>
      <CurrencyIcon type={'primary'} />
